...
“契约”可以理解为“双方必须遵守的接口约定”。  
在本项目里，契约主要指：
- 输出有几维（`OUTPUT_DIM`）
- 每一维代表什么动作（`IO_MAPPING_NAMES`）
- 每一维输出值域是什么（`IO_MAPPING_ACTIVATIONS`）
