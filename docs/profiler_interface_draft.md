...
补充约束：
- 用户侧若传入网络类型专属配置，应通过 config-only 头文件定义对应 C 类型。
- 用户侧不直接包含 `*_infer_ops.h` / `*_train_ops.h`。
- 生成代码可以包含该 config-only 头文件，并把具体配置类型耦合到生成的 `.c` 中。
...
- 接口类型与实现必须可在 C11 编译模式下通过。
- 不使用超出 C11 的语言特性。
- 固定头文件模板与生成 `.c` 一并遵守 C11。
