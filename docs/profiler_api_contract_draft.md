...
## 4.4 nn_register_subnet_serde

| 参数 | 必填 | 约束 | 默认行为 |
|---|---|---|---|
| vtable | 是 | 非空，包含 network_type/save_serialize/load_deserialize | 无效返回 INVALID_ARGUMENT |

附加规则：

- 注册在编译期装配阶段完成。
- 网络类型编译启用由 CMakeLists 开关控制。
- `save_serialize` 指向训练实现；`load_deserialize` 指向推理实现。
- 新网络类型接入只允许新增注册项，不允许修改既有对外 API。
- 新网络类型接入只允许修改注册配置文件或注册宏清单，不允许改动 profiler 主流程。
- 未启用网络类型不得进入注册表与编译产物。
- 通过 CMake 开关与注册流程保证“未启用类型不会出现”，不新增该类运行时错误分支。
...
- 源码与 profiler 生成代码的注释量必须不低于 50%（必须遵守）。

## 10. 编译质量约束

- 必须启用编译器最严格校验选项。
- 必须启用警告即错误策略（Warnings as Errors）。
- 编译结果必须为零警告。

## 11. 问题处理原则

- 处理问题优先采用流程、过程、结构层面的改进方案。
- 流程正确性优先通过机制保证，数据验证只用于校验数据符合流程约束。
- 不以数据补丁式修复作为主方案。
- 如存在方案取舍或不合理设计风险，必须先请示用户。
- 扩展网络类型采用统一配置与注册流程，不通过改旧代码路径完成适配。
