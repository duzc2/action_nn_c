 ...
 * @brief Thin cache over the inference registry for profiler/codegen use.
 *
 * After the VTable migration (B3), this layer is now a straightforward cache
 * that derives graph-capability flags from raw registry entries. The profiler
 * and generated code still depend on NNGraphInferContract struct layouts.
 */
...
const NNGraphInferContract* nn_graph_infer_contract_find(const char* type_name) {
    ...
    /* Return cached contract when available. */
    ...
    /* Materialize from registry entry on cache miss. */
    ...
    for (index = 0; index < (int)(sizeof(g_infer_contract_slots) / sizeof(g_infer_contract_slots[0])); ++index) {
        if (!g_infer_contract_slots[index].used) {
            ...
            if (name_len >= sizeof(g_infer_contract_slots[index].type_name)) {
                name_len = sizeof(g_infer_contract_slots[index].type_name) - 1U;
            }
            g_infer_contract_slots[index].used = 1;
            (void)memcpy(g_infer_contract_slots[index].type_name, type_name, name_len);
            g_infer_contract_slots[index].type_name[name_len] = '\0';
            ...
            g_infer_contract_slots[index].contract.supports_graph_mode = (entry->graph_run != 0) ? 1 : 0;
            return &g_infer_contract_slots[index].contract;
        }
    }
