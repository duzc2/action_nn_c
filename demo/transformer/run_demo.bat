...
set "BUILD_ROOT=%ACTION_C_ROOT%\build\demo\transformer"

taskkill /F /IM transformer_generate.exe >nul 2>&1
taskkill /F /IM transformer_train.exe >nul 2>&1
taskkill /F /IM transformer_infer.exe >nul 2>&1
...
set "RUN_COMMAND=""%BUILD_ROOT%\generate\Debug\transformer_generate.exe"""
set "RUN_TIMEOUT=30"
...
set "RUN_COMMAND=""%BUILD_ROOT%\train\Debug\transformer_train.exe"""
set "RUN_TIMEOUT=30"
call :run_with_timeout
if errorlevel 1 goto :fail
...
set "RUN_COMMAND=(echo hello ^& echo quit) ^| ""%BUILD_ROOT%\infer\Debug\transformer_infer.exe"""
set "RUN_TIMEOUT=30"
call :run_with_timeout
if errorlevel 1 goto :fail
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "catch { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue; Write-Host '[transformer] process timeout'; exit 124 }"
