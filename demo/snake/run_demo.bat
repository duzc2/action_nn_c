...
set "BUILD_ROOT=%ACTION_C_ROOT%\build\demo\snake"

taskkill /F /IM snake_generate.exe >nul 2>&1
taskkill /F /IM snake_train.exe >nul 2>&1
taskkill /F /IM snake_infer.exe >nul 2>&1
...
set "RUN_COMMAND=""%BUILD_ROOT%\generate\Debug\snake_generate.exe"""
set "RUN_TIMEOUT=30"
...
set "RUN_COMMAND=""%BUILD_ROOT%\train\Debug\snake_train.exe"""
set "RUN_TIMEOUT=120"
call :run_with_timeout
if errorlevel 1 goto :fail
...
echo [snake] step 6/6 run infer
set "RUN_COMMAND=""%BUILD_ROOT%\infer\Debug\snake_infer.exe"""
set "RUN_TIMEOUT=60"
call :run_with_timeout
if errorlevel 1 goto :fail

echo [snake] demo completed successfully
exit /b 0

:fail
echo [snake] demo failed
exit /b 1
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "catch { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue; Write-Host '[snake] process timeout'; exit 124 }"
